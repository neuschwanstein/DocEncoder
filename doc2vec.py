...
def doc2vec(q_w, q_p, batch_size=200, steps=10000, k=12, db_limit=100):
    ...
    # Index placeholders
    t_cs_dmm = tf.placeholder(tf.int32, shape=[n,1])
    ...
    h_ps = tf.gather(D_dmm, t_ps_dmm,name='h_ps_gather')
    h_ws = tf.gather(W_dmm, t_ws_dmm,name='h_ws_gather')
    ...
    U_dmm = tf.Variable(tf.zeros([T_w,q_w+q_p]),name='U_dmm')
    b_dmm = tf.Variable(tf.zeros([T_w]),name='b_dmm')

    cost_dmm = tf.reduce_mean(
        tf.nn.sampled_softmax_loss(U_dmm,b_dmm,h_dmm,t_cs_dmm,num_sampled_dmm,T_w))
    train_dmm = tf.train.GradientDescentOptimizer(0.1).minimize(cost_dmm)

    # Distributed bag of words
    D_bow = tf.Variable(tf.random_uniform([T_p,q_p], -1.0, 1.0),name='D_bow')

    # Index placeholders
    t_ps_bow = tf.placeholder(tf.int32, shape=[n])
    t_css_bow = tf.placeholder(tf.int32, shape=[n,2*k])
    
    # PVBOW vector
    h_bow = tf.gather(D_bow, t_ps_bow,name="my_gather_motherfucker") # [n,q_p]

    # Softmax paramaters for PVDBOW model
    U_bow = tf.Variable(tf.zeros([T_w,q_p]),name='U_bow')
    b_bow = tf.Variable(tf.zeros([T_w]),name='b_bow')

    num_sampled = 64            # How much??
    cost_bow = tf.reduce_mean(
        tf.nn.sampled_softmax_loss(U_bow,b_bow,h_bow,t_css_bow,num_sampled_bow,T_w,2*k))
    ...
    with tf.Session() as sess:
        ...
        for i in range(steps):
            # PVDM training
            feed_dmm = dict(zip(
                [t_ps_dmm,t_ws_dmm,t_cs_dmm], stochastic_batch_dmm(k,n)))
...
def stochastic_batch_dmm(k, n=200):
    ...
    t_cs = np.array(t_cs).reshape((len(t_ps),1))
